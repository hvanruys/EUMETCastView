...
{
    sm->ComposeSegmentImagenetCDFMTGInThreadConcurrent();  //Concurrent : ComposeSegmentImagenetCDFMTGInThreadConcurrent() ; Serial : ComposeSegmentImagenetCDFMTGInThreadSerial()
}                                                 //             ComposeSegmentImagenetCDFMTGInThread1()
...
void SegmentListGeostationary::setGeoSatellite(int geoindex)
{
    ...
    else if(str_GeoSatellite == "GOES_19")
    {
...
void SegmentListGeostationary::getFilenameParameters(QString file, QString *filespectrum, QString *filedate, int *filesequence, int *channelindex)
{
    ...
    else
    {
        ...
        else if(spectrumvector.at(3) == *filespectrum)
        {
            *channelindex = 3;
        }
    }

    //qDebug() << "SegmentListGeostationary::getFilenameParameters at geoindex = "
    //         << geoindex << " channelindex = " << *channelindex << " date = " << *filedate;
...
{
    //qDebug() << QString("SegmentListGeostationary::ComposeImagenetCDFMTGInThread spectrumvector = %1 %2 %3 %4")
    //            .arg(spectrumvector.at(0)).arg(spectrumvector.at(1)).arg(spectrumvector.at(2)).arg(spectrumvector.at(3));
...
void SegmentListGeostationary::ComposeSegmentImagenetCDFInThread() //(QStringList filelist, QVector<QString> spectrumvector, QVector<bool> inversevector , int histogrammethod)
{
    ...
    for(int i = 0; i < this->spectrumvector.count(); i++)
    {
        ...
        {
            if(this->segmentfilelist[j].mid(opts.geosatellites.at(geoindex).indexspectrum, opts.geosatellites.at(geoindex).spectrumlist.at(0).length()) ==
                    this->spectrumvector.at(i))
...
void SegmentListGeostationary::ComposeSegmentImagenetCDFMTGInThreadSerial() // Serial
{
    ...
    qDebug() << "Start SegmentListGeostationary::ComposeSegmentImagenetCDFMTGInThreadSerial() size = " << this->segmentfilelist.size();
...
void SegmentListGeostationary::concurrentReadFilelist(SegmentListGeostationary *sm, QString llFile)
{
    ...
    qDebug() << QString("concurrentReadFilelist() %1 channelindex = %2 filesequence = %3")
    ...
    else
    {
        ...
        {
            imageptrs->ptrNight[filesequence] = new quint16[nlin * npix];
            memset(imageptrs->ptrNight[filesequence], 0, nlin * npix * sizeof(quint16));
        }
        ...
        for (int pixelx = 0 ; pixelx < npix; pixelx++)
        {
            ...
            else
            {
                if(channelindex == 0)
                    ...
                    *(imageptrs->ptrNight[filesequence] + line * npix + pixelx) = c;
            }
        }
    }
...
void SegmentListGeostationary::ComposeSegmentImagenetCDFMTGInThreadConcurrent()
{
    ...
    QVector<int> vec_findex;
    QVector<QString> vec_fname;
    ...
    for(int j = 0; j < this->segmentfilelist.size(); j++)
    {
        if(this->segmentfilelist.at(j).contains("BODY"))
        {
            ...
            int ind = filePath.indexOf(".nc");
            int findex = filePath.mid(ind - 4, 4).toInt();
            vec_findex.append(findex);
            vec_fname.append(filePath);

        }
    }

// #ifdef CONC
//     auto cbReadMTGfile = std::bind(this->ReadConcurrentMTGfile, this, std::placeholders::_1);
//     QtConcurrent::blockingMap(vec_fname, cbReadMTGfile);
// #else
    for(int i = 0; i < vec_findex.length(); i++)
    {
        this->ReadConcurrentMTGfile(this, vec_fname[i]);
    }
//#endif
...
#ifdef CONC
    auto cbMinMaxMTG = std::bind(this->concurrentMinMaxMTG, this, std::placeholders::_1);
    QtConcurrent::blockingMap(vec_findex, cbMinMaxMTG);
#else
    for(int i = 0; i < vec.length(); i++)
    {
        this->concurrentMinMaxMTG(this, vec_findex[i]);
    }
#endif
    ...
    for(int i = 0; i < (this->kindofimage == "VIS_IR Color" ? (this->spectrumvector.at(3).length() > 0 ? 4 : 3) : 1); i++) {
        for (int j = 0; j < vec_findex.length(); j++)
        {
            quint16 val = imageptrs->mtg_stat_min[i][vec_findex[j]-1];
            if(val != imageptrs->fillvalue[i])
            {
                if(val < stat_min[i])
                    stat_min[i] = val;
            }
            val = imageptrs->mtg_stat_max[i][vec_findex[j]-1];
            if(val != 0)
            {
                if(val > stat_max[i])
                    stat_max[i] = val;
...
#ifdef CONC
    auto cbLUTMTG = std::bind(this->concurrentLUTGeoMTG, this, std::placeholders::_1);
    QtConcurrent::blockingMap(vec_findex, cbLUTMTG);
#else
    for(int i = 0; i < vec.length(); i++)
    {
        this->concurrentLUTGeoMTG(this, vec_findec[i]);
    }
#endif
    ...
    {
        this->active_pixels[colorindex] = 0;
        for (int index = 0; index < vec_findex.length(); index++)
        {
            this->active_pixels[colorindex] += imageptrs->mtg_active_pixels[colorindex][vec_findex[index]-1];
        }
        qDebug() << QString("active_pixels[%1] = %2").arg(colorindex).arg(this->active_pixels[colorindex]);
    }
    ...
    for(int colorindex = 0; colorindex < (this->kindofimage == "VIS_IR Color" ? (this->spectrumvector.at(3).length() > 0 ? 4 : 3) : 1); colorindex++)
    {
        for (int index = 0; index < vec_findex.length(); index++) {
            for (int i = 0; i < 4096; i++) {
                histogram[colorindex][i] += imageptrs->mtg_histogram[colorindex][vec_findex[index]-1][i];
                totpixels[colorindex] += imageptrs->mtg_histogram[colorindex][vec_findex[index]-1][i];
            }
        }
        qDebug() << QString("totpixels[%1] = %2 active_pixels[%3] = %4").arg(colorindex).arg(totpixels[colorindex]).arg(colorindex).arg(this->active_pixels[colorindex]);
    }
    ...
    if(this->spectrumvector.at(3).length() > 0)
    {
        ...
        QtConcurrent::blockingMap(vec_findex, cbImageNight);
#else
        for(int i = 0; i < vec.length(); i++)
        {
            this->concurrentImageMTGNight(this, vec[i]);
...
#ifdef CONC
    auto cbImageMTG = std::bind(this->concurrentImageMTG, this, std::placeholders::_1);
    QtConcurrent::blockingMap(vec_findex, cbImageMTG);
#else
    for(int i = 0; i < vec.length(); i++)
    {
        this->concurrentImageMTG(this, vec[i]);
    ...
    for(int i = 0; i < (kindofimage == "VIS_IR Color" ? (this->spectrumvector.at(3).length() > 0 ? 4 : 3) : 1); i++)
    {
        for(int j = 0; j < vec_findex.length(); j++)
        {
            if(imageptrs->ptrMTG[i][vec_findex[j]-1] != NULL)
            {
                delete [] imageptrs->ptrMTG[i][vec_findex[j]-1];
                imageptrs->ptrMTG[i][vec_findex[j]-1] = NULL;
            }
            if(imageptrs->ptrIndex[i][vec_findex[j]-1] != NULL)
            {
                delete [] imageptrs->ptrIndex[i][vec_findex[j]-1];
                imageptrs->ptrIndex[i][vec_findex[j]-1] = NULL;
            }
        }
    }
...
void SegmentListGeostationary::ReadConcurrentMTGfile(SegmentListGeostationary *sm, const QString &filenamepath)
{
    int ncfileid;
    int grp_data;
    int grp_channel;
    ...
    if(filenamepath.contains("BODY"))
    {
        QByteArray arrayncfile;
        const char* pncfile;


        arrayncfile = filenamepath.toUtf8();
        pncfile = arrayncfile.constData();

        qDebug() << "Starting netCDF file " + filenamepath;
        int ind = filenamepath.indexOf(".nc");
        int findex = filenamepath.mid(ind - 4, 4).toInt();

        //vec.append(findex);

        retval = nc_open(pncfile, NC_NOWRITE, &ncfileid);
        if(retval != NC_NOERR) qDebug() << "error opening netCDF file " << filenamepath;
        ...
        qDebug() << QString("geospatial lat min = %1 lat max = %2 ").arg(geospatial_lat_min).arg(geospatial_lat_max);
        ...
        for(int i = 0; i < (sm->kindofimage == "VIS_IR Color" ? (sm->spectrumvector.at(3).length() > 0 ? 4 : 3) : 1); i++)
        {
            qDebug() << "reading radiance from channel i = " << i << " spectrum = " << sm->spectrumvector.at(i);
            ...
            QString strmeasured = "/data/" + sm->spectrumvector.at(i) + "/measured";
            ba = strmeasured.toLocal8Bit();
            const char *c_measured = ba.data();
            ...
            QString strradiance = "/data/" + sm->spectrumvector.at(i) + "/measured/effective_radiance";
            ba = strradiance.toLocal8Bit();
            const char *c_radiance = ba.data();
            ...
            if(opts.bFciDecomp)
                ...
                retval = sm->read_charls_compressed_ushort(pncfile, c_radiance, grp_measured,  varid, imageptrs->ptrMTG[i][findex - 1]);
            ...
            QString strindex_map = "/data/" + sm->spectrumvector.at(i) + "/measured/index_map";
            ba = strradiance.toLocal8Bit();
            const char *c_index_map = ba.data();
            ...
            if(opts.bFciDecomp)
                ...
                retval = sm->read_charls_compressed_ushort(pncfile, c_index_map, grp_measured, varid, imageptrs->ptrIndex[i][findex - 1]);

            if(retval != NC_NOERR) qDebug() << "error reading index_map from channel " << strmeasured << " findex = " << findex << " error = " << retval;

        }
        retval = nc_close(ncfileid);
        if (retval != NC_NOERR) qDebug() << "error closing file " << filenamepath;
        qDebug() << "cleaning up " << filenamepath;

        if(opts.copyMTGfiles)
            QFile::remove(filenamepath);

        emit sm->progressCounter(sm->progcounter += 1);
...
int SegmentListGeostationary::concurrentMinMaxMTG(SegmentListGeostationary *sm, const int &index)
{
    ...
    for(int j = 0; j < (sm->kindofimage == "VIS_IR Color" ? (sm->spectrumvector.at(3).length() > 0 ? 4 : 3) : 1); j++)
    {
...
int SegmentListGeostationary::concurrentImageMTGNight(SegmentListGeostationary *sm, const int &index)
{
    sm->CalculateImageMTGConcurrentNight(index);
    return(index);
}

QString SegmentListGeostationary::concurrentReadMTGfile(SegmentListGeostationary *sm, const QString &filename)
{
    sm->ReadConcurrentMTGfile(sm, filename);
    return(filename);
}
...
{
    qDebug() << QString("start CalculateLUTGeo colorindex = %1").arg(colorindex);
    ...
    for (int line = 0; line < opts.geosatellites[geoindex].imageheight; line++)
    {
        ...
        {
            pixel = ptr[line * opts.geosatellites[geoindex].imagewidth + pixelx];
...
void SegmentListGeostationary::CalculateImageMTGConcurrent(int index)
{
    ...
    long coff = opts.geosatellites[geoindex].coffhrv;
    long loff = opts.geosatellites[geoindex].loffhrv;
    double cfac = opts.geosatellites[geoindex].cfachrv;
    double lfac = opts.geosatellites[geoindex].lfachrv;
...
void SegmentListGeostationary::CalculateImageMTGConcurrentNight(int index)
{
    quint16 pixel;
    ...
    if(this->spectrumvector.at(3).length() > 0)
    {
        for(int line = imageptrs->mtg_start_position_row[3][index-1] - 1; line < imageptrs->mtg_end_position_row[3][index-1]; line++)
        {
            for (int pixelx = imageptrs->mtg_start_position_column[3][index-1] - 1; pixelx < imageptrs->mtg_end_position_column[3][index-1]; pixelx++)
            {
                pixel = *(imageptrs->ptrMTG[3][index-1] + ((imageptrs->mtg_nbr_of_columns[3][index-1]) * linelocal) + pixelx);

                if(pixel != imageptrs->fillvalue[3])
                {
                    if(this->histogrammethod == CMB_HISTO_NONE_95)
                    {
                        if(pixel != imageptrs->fillvalue[3])
                        {
                            indexoutpixel = (quint16)qMin(qMax(qRound(1023.0 * (float)(pixel - imageptrs->minRadianceIndex[3] ) / (float)(imageptrs->maxRadianceIndex[3] - imageptrs->minRadianceIndex[3])), 0), 1023);
                    ...
                    else if(this->histogrammethod == CMB_HISTO_EQUALIZE)
                    {
                        if( pixel != imageptrs->fillvalue[3])
                        {
                            ...
                            indexoutpixel = (quint16)(qMin(qMax((int)imageptrs->lut_mtg[3][val], 0), 4095));
...
void SegmentListGeostationary::CalculateImageMTGConcurrentAlt(int index)
{
    quint16 pixel[4];
    quint16 indexoutpixel[4];
    quint8 pixelout[4];
    ...
    im_geo = imageptrs->ptrimageGeostationary;

    QImage im_night_scaled;
    ...
    if(this->spectrumvector.at(3).length() > 0)
    {
        im_night = imageptrs->ptrimageGeostationaryNight;

        qDebug() << "index = " << index << " im_night width = " << im_night->width() << " height = " << im_night->height();

        for(int line = imageptrs->mtg_start_position_row[3][index-1] - 1; line < imageptrs->mtg_end_position_row[3][index-1]; line++)
        {
            row_col_night = (QRgb*)im_night->scanLine(imageptrs->mtg_total_number_of_rows[3] - 1 - line);

            for (int pixelx = imageptrs->mtg_start_position_column[3][index-1] - 1; pixelx < imageptrs->mtg_end_position_column[3][index-1]; pixelx++)
            {
                pixel[3] = *(imageptrs->ptrMTG[3][index-1] + ((imageptrs->mtg_nbr_of_columns[3][index-1]) * linelocal_night) + pixelx);

                if(pixel[3] != imageptrs->fillvalue[3])
                {
                    if(this->histogrammethod == CMB_HISTO_NONE_95)
                    {
                        if(pixel[3] != imageptrs->fillvalue[3])
                        {
                            indexoutpixel[3] = (quint16)qMin(qMax(qRound(1023.0 * (float)(pixel[3] - imageptrs->minRadianceIndex[3] ) / (float)(imageptrs->maxRadianceIndex[3] - imageptrs->minRadianceIndex[3])), 0), 1023);
                            valgamma = pow( indexoutpixel[3], gamma) * gammafactor;
                            if (valgamma > 1023)
                                valgamma = 1023;

                            valcontrast = ContrastStretch(valgamma);
                            pixelout[3] = (valcontrast);
                        }
                    }
                    else if(this->histogrammethod == CMB_HISTO_NONE_100)
                    {
                        if(pixel[3] != imageptrs->fillvalue[3])
                        {
                            indexoutpixel[3] = (quint16)qMin(qMax(qRound(1023.0 * (float)(pixel[3] - this->stat_min[3] ) / (float)(this->stat_max[3] - this->stat_min[3])), 0), 1023);
                            valgamma = pow( indexoutpixel[3], gamma) * gammafactor;
                            if (valgamma > 1023)
                                valgamma = 1023;
                    ...
                    else if(this->histogrammethod == CMB_HISTO_EQUALIZE)
                    {
                        if( pixel[3] != imageptrs->fillvalue[3])
                        {
                            quint16 val = (quint16)qMin(qMax(qRound(4095.0 * (float)(pixel[3] - this->stat_min[3] ) / (float)(this->stat_max[3] - this->stat_min[3])), 0), 4095);
                            indexoutpixel[3] = (quint16)(qMin(qMax((int)imageptrs->lut_mtg[3][val], 0), 4095));
                            valgamma = pow( indexoutpixel[3]/4, gamma) * gammafactor;
                            if (valgamma > 1023)
                                valgamma = 1023;
                    ...
                    pixelout[3] = quint16(this->inversevector[3] ? (255 - pixelout[3]) : pixelout[3]);
                    row_col_night[pixelx] = qRgb(pixelout[3], pixelout[3], pixelout[3]);
                }
        ...
        im_night_scaled = im_night->scaledToWidth( 11136, Qt::SmoothTransformation );
    }
...
*/
    ...
    for(int line = imageptrs->mtg_start_position_row[0][index-1] - 1; line < imageptrs->mtg_end_position_row[0][index-1]; line++)
    {
        row_col_geo = (QRgb*)im_geo->scanLine(imageptrs->mtg_total_number_of_rows[0] - 1 - line);

        if(this->spectrumvector.at(3).length() > 0)
            row_col_night_scaled = (QRgb*)im_night_scaled.scanLine(imageptrs->mtg_total_number_of_rows[0] - 1 - line);
        ...
        for (int pixelx = imageptrs->mtg_start_position_column[0][index-1] - 1; pixelx < imageptrs->mtg_end_position_column[0][index-1]; pixelx++)
        {
            for(int colorindex = 0; colorindex < (this->kindofimage == "VIS_IR Color" ? 3 : 1); colorindex++)
            {
                ...
                else if(this->histogrammethod == CMB_HISTO_EQUALIZE)
                {
                    if( pixel[colorindex] != imageptrs->fillvalue[colorindex])
                    {
                        quint16 val = (quint16)qMin(qMax(qRound(4095.0 * (float)(pixel[colorindex] - this->stat_min[colorindex] ) / (float)(this->stat_max[colorindex] - this->stat_min[colorindex])), 0), 4095);
                        indexoutpixel[colorindex] = (quint16)(qMin(qMax((int)imageptrs->lut_mtg[colorindex][val], 0), 4095));
                        valgamma = pow( indexoutpixel[colorindex]/4, gamma) * gammafactor;
                        if (valgamma > 1023)
                            valgamma = 1023;
            ...
            if(this->kindofimage == "VIS_IR")
            {
                if(pixel[0] != imageptrs->fillvalue[0])
                {
                    ...
                    pixelout[1] = pixelout[0];
                    pixelout[2] = pixelout[0];
                    row_col_geo[pixelx] = qRgb(pixelout[0], pixelout[1], pixelout[2]);
                }
            }
            else if(this->kindofimage == "VIS_IR Color")
            {
                if(this->spectrumvector.at(3).length() > 0)
                {
                    ret = pixconv.pixcoord2geocoord(sub_lon, pixelx,
                    ...
                    if(pixel[0] != imageptrs->fillvalue[0])
                    {
                        if(ret > -1)
                        {
                            ...
                            if(elev <= 0.0)
                            {
                                row_col_geo[pixelx] = row_col_night_scaled[pixelx];
                                //row_col[pixelx] = qRgb(255, 0, 0);
                            }
                            else
                            {
...
void SegmentListGeostationary::ComposeVISIR()
{
    ...
    if(kindofimage == "VIS_IR Color")
    {
        ...
        if(this->spectrumvector.at(3).length() > 0)
            pixelsNight = new quint16[nbrpix];
    ...
    if(kindofimage == "VIS_IR Color")
    {
        ...
        if(this->spectrumvector.at(3).length() > 0)
        {
            for( int i = (bisRSS ? 5 : 0); i < opts.geosatellites[geoindex].maxsegments + (bisRSS ? 5 : 0); i++)
            {
                if(imageptrs->ptrNight[i] != NULL)
                    memcpy(pixelsNight + (bisRSS ? i - 5 : i) * nbroflinespersegment * opts.geosatellites[geoindex].imagewidth, imageptrs->ptrNight[i], nbroflinespersegment * opts.geosatellites[geoindex].imagewidth * sizeof(quint16));
                else
                    memset(pixelsNight + (bisRSS ? i - 5 : i) * nbroflinespersegment * opts.geosatellites[geoindex].imagewidth, 0, nbroflinespersegment * opts.geosatellites[geoindex].imagewidth * sizeof(quint16));
    ...
    if(kindofimage == "VIS_IR Color")
    {
        ...
        if(this->spectrumvector.at(3).length() > 0)
        {
            CalculateMinMax(3, width, height, pixelsNight, 0);
            CalculateLUTGeo(3, pixelsNight, 0);
            //equalizeHistogram(pixelsNight, width, height, 3, 0, 1023);
            imageptrs->CLAHE(pixelsNight, 3712, 3712, 0, 1023, 16, 16, 256, 5);

        }
    }

    computeGeoImageVISIR(pixelsRed, pixelsGreen, pixelsBlue, pixelsNight);
...
void SegmentListGeostationary::computeGeoImageVISIR(quint16 *pixelsRed, quint16 *pixelsGreen, quint16 *pixelsBlue, quint16 *pixelsNight)
{
    QRgb *row_col;
    QRgb *row_col_bitmap;
    ...
    for (int line = opts.geosatellites[geoindex].maxsegments*nbroflinespersegment - 1; line >= 0; line--)
    {
        ...
        for (int pixelx = opts.geosatellites[geoindex].imagewidth - 1; pixelx >= 0; pixelx--)
        {
            if(kindofimage == "VIS_IR Color")
            {
                ...
                if(this->histogrammethod == CMB_HISTO_NONE_95)
                {
                    ...
                    if(pixelsNight != NULL)
                    {
                        if(cnight < 65528)
                        {
                            ...
                            //indexoutnc = (quint16)qMin(qMax(qRound(1023.0 * (float)(cnight - this->stat_min[3] ) / (float)(this->stat_max[3] - this->stat_min[3])), 0), 1023);
                            //indexoutnc = (quint16)qMin(qMax(qRound((float)imageptrs->lut_ch[3][indexoutnc]), 0), 1023);
                ...
                ret = pixconv.pixcoord2geocoord(sub_lon, opts.geosatellites[geoindex].imagewidth - 1 - pixelx,
                                                opts.geosatellites[geoindex].imageheight - 1 - line, coff, loff, cfac, lfac, &latitude, &longitude);
                if(ret == -1)
                ...
                else
                {
                    ...
                    else if(elev > 0.0 && elev < twilight)
                    {
                        int percentday = (int)(100.0 * elev / twilight);
                        int percentnight = 100 - percentday;
                        int red = (percentday*r + percentnight*n)/100;
                        red = (red > 255 ? 255 : red);

                        int green = (percentday*g + percentnight*n)/100;
                        green = (green > 255 ? 255 : green);

                        int blue = (percentday*b + percentnight*n)/100;
                        blue = (blue > 255 ? 255 : blue);
                        if(opts.geosatellites[geoindex].rss)
                        {
                            if( (opts.geosatellites[geoindex].rss ? 1392 : 3712) - 100 < opts.geosatellites[geoindex].imageheight - 1 - line)
                                row_col[opts.geosatellites[geoindex].imagewidth - 1 - pixelx] = qRgba(0, 0, 0, (imageptrs->alphazero == true ? 0 : 255));
                            else
                            {
...
void SegmentListGeostationary::computeGeoImageHimawari(quint16 *pixelsRed, quint16 *pixelsGreen, quint16 *pixelsBlue, quint16 *pixelsNight)
{
    QRgb *row_col;
    QRgb *row_col_bitmap;
    quint16 cred, cgreen, cblue, cnight;
...
void SegmentListGeostationary::ComposeVISIRHimawari()
{
    ...
    QRgb *row_col;
    quint16 cred, cgreen, cbluen, cnight;
    quint16 r, g, b, n;
    quint16 valcontrast;
    ...
    computeGeoImageHimawari(pixelsRed, pixelsGreen, pixelsBlue, pixelsNight);
#else
...
{
    qDebug() << "==========start ComposeHRV geoindex = " << geoindex << " ==================";



    //    int LECA = 11136 - LowerEastColumnActual;
    //    int LSLA = 11136 - LowerSouthLineActual;
    //    int LWCA = 11136 - LowerWestColumnActual;
    //    int LNLA = 11136 - LowerNorthLineActual;

    //    int UECA = 11136 - UpperEastColumnActual;
    //    int USLA = 11136 - UpperSouthLineActual;
    //    int UWCA = 11136 - UpperWestColumnActual;
    //    int UNLA = 11136 - UpperNorthLineActual;
    ...
    // long countFiller = 0;
    // for( int i = 0; i < npixHRV; i++)
    // {
    //     if(*(pixelsHRV + i) == 0)
    //         countFiller++;
    // }
    // qDebug() << "============> count zero befor clahe = " << countFiller;
    ...
    if(kindofimage == "HRV Color")
    {
        ...
        if(this->spectrumvector.at(3).length() > 0)
        {
            ...
            {
                if(imageptrs->ptrNight[i] != NULL)
                    memcpy(pixelsNight + i * 464 * 3712, imageptrs->ptrNight[i], 464 * 3712 * sizeof(quint16));
                else
                    memset(pixelsNight + i * 464 * 3712, 0, 464 * 3712 * sizeof(quint16));
            }
        }
    }
    ...
    ////            CalculateLUTGeo(3, pixelsNight, 0);
    ////            equalizeHistogram(pixelsNight, width, height, 3, 0, 1023);
    //            imageptrs->CLAHE(pixelsNight, 3712, 3712, 0, 1023, 16, 16, 256, 3);
    //        }
    //    }


    //imageptrs->CLAHE(pixelsHRV, 5568, (opts.geosatellites.at(geoindex).rss ? 5 : (this->areatype == 1 ? 24 : 5)) * 464, 0, 1023, 16, 16, 256, 4);
    ...
    //         countFiller++;
    // }
    // qDebug() << "============> count zero after clahe = " << countFiller;
    ...
    if(kindofimage == "HRV Color")
    {
        ...
        CalculateMinMax(4, 5568, (opts.geosatellites.at(geoindex).rss || this->areatype == 0 ? 5 * 464 : 11136 ), pixelsHRV, 0);

    }

    computeGeoImageHRV(pixelsRed, pixelsGreen, pixelsBlue, pixelsNight, pixelsHRV);
...
void SegmentListGeostationary::computeGeoImageHRV(quint16 *pixelsRed, quint16 *pixelsGreen, quint16 *pixelsBlue, quint16 *pixelsNight, quint16 *pixelsHRV)
{
    QRgb *row_col;
    QRgb *row_col_bitmap;

    quint16 cred, cgreen, cblue, cnight, c, clum;
    quint16 r, g, b, n;
    ...
    long coffhrv = opts.geosatellites[geoindex].coffhrv;
    long loffhrv = opts.geosatellites[geoindex].loffhrv;
    double cfachrv = opts.geosatellites[geoindex].cfachrv;
    double lfachrv = opts.geosatellites[geoindex].lfachrv;
    ...
    qDebug() << "SegmentListGeostationary::computeGeoImageHRV kindofimage = " << this->kindofimage;
        ...
        for (int pixelx = 5568 - 1 ; pixelx >= 0; pixelx--)
        {
            ...
            if(this->kindofimage == "HRV Color")
            {
                ...
                else
                {
                    if(this->areatype == 0)
                    {
                        int diff = (19*464 + line)/3 * 3712 + UpperEastColumnActual/3 + pixelx/3;
                        cred = *(pixelsRed + diff);
                        cgreen = *(pixelsGreen + diff);
                        cblue = *(pixelsBlue + diff);
                        if(this->spectrumvector.at(3).length() > 0)
                    ...
                    else
                    {
                        ...
                        {
                            int diff = line/3 * 3712 + UpperEastColumnActual/3 + pixelx/3;
                            cred = *(pixelsRed + diff);
                            cgreen = *(pixelsGreen + diff);
                            cblue = *(pixelsBlue + diff);
                            if(this->spectrumvector.at(3).length() > 0)
                        ...
                        {
                            int diff = line/3 * 3712 + LowerEastColumnActual/3 + pixelx/3;
                            cred = *(pixelsRed + diff);
                            cgreen = *(pixelsGreen + diff);
                            cblue = *(pixelsBlue + diff);
                            if(this->spectrumvector.at(3).length() > 0)
                ...
                {
                    c = c*2;
                    valgamma = pow( c*0.7*cred/clum, gamma) * gammafactor;
                    if (valgamma >= 1024)
                        valgamma = 1023;
                ...
                if(this->spectrumvector.at(3).length() > 0)
                {
                    ...
                    if (n > 255)
                        n = 255;
                    (this->inversevector.at(3) ? n = 255 - n : n);
                }

                //imageheight=1392
                //imageheighthrv0=2320
                //imageheighthrv1=11136
                //imagewidth=3712
                //imagewidthhrv0=5568
                //imagewidthhrv1=5568
                ...
                if(this->spectrumvector.at(3).length() > 0)
                {
                    if(this->areatype == 0)
                    {
                        row = opts.geosatellites[geoindex].imageheighthrv0 - 1 - line;
                        col = opts.geosatellites[geoindex].imagewidthhrv0 - 1 - pixelx;
                        if (row > 11136 - LowerNorthLineActual ) //LOWER
                        {
                            col = col + (11136 - 1 - LowerWestColumnActual);
                        }
                        else //UPPER
                        {
                            col = col + (11136 - 1 - UpperWestColumnActual);
                        }

                        ret = pixconv.pixcoord2geocoord(sub_lon, col, row, coffhrv, loffhrv, cfachrv, lfachrv, &latitude, &longitude);
                    }
                    else
                    {
                        row = opts.geosatellites[geoindex].imageheighthrv1 - 1 - line;
                        col = opts.geosatellites[geoindex].imagewidthhrv1 - 1 - pixelx;
                        if (row > 11136 - LowerNorthLineActual ) //LOWER
                        {
                            col = col + (11136 - 1 - LowerWestColumnActual);
                        }
                    ...
                    if(ret == -1)
                        row_col[opts.geosatellites[geoindex].imagewidthhrv0 - 1 - pixelx] = qRgba(0, 0, 0, (imageptrs->alphazero == true ? 0 : 255));
                    else
                    {
                        observer.SetLocation(latitude, longitude, 0.0);
                        dat.Set(year, month, day, hours, minutes, 0, true);
                        ...
                        if(elev <= - 6.0)
                        {
                            row_col[opts.geosatellites[geoindex].imagewidthhrv0 - 1 - pixelx] = qRgb(n, n, n);
                        }
                        else if(elev > - 6.0 && elev < 6.0)
                        {
                            double abs_elev = elev + 6.0;
                            int percentday = (int)(100.0 * abs_elev / twilight);
                            int percentnight = 100 - percentday;
                            int red = (percentday*r + percentnight*n)/100;
                            red = (red > 255 ? 255 : red);
                            ...
                            //                            if(qRed(row_col_bitmap[opts.geosatellites[geoindex].imagewidthhrv0 - 1 - pixelx]) == 0)
                            //                            {
                            //                                red = 0;
                            //                                green = 0;
                            //                                blue = 0;
                            ...
                            if(opts.geosatellites[geoindex].rss)
                            {
                                if( (opts.geosatellites[geoindex].rss ? 1392 : 3712) - 100 < opts.geosatellites[geoindex].imageheighthrv0 - 1 - line)
                                    row_col[opts.geosatellites[geoindex].imagewidthhrv0 - 1 - pixelx] = qRgba(0, 0, 0, (imageptrs->alphazero == true ? 0 : 255));
                                else
                                {
                                    if(red == 0 && green == 0 && blue == 0)
                                        ...
                                        row_col[opts.geosatellites[geoindex].imagewidthhrv0 - 1 - pixelx] = qRgb(red, green, blue);
                                }
                            }
                            else
                            {
                            ...
                            row_col[opts.geosatellites[geoindex].imagewidthhrv0 - 1 - pixelx] = qRgb(r, g, b);
...
void SegmentListGeostationary::ComposeHRV_Alt()
{
    QRgb *row_col;
    QRgb *row_col_bitmap;
    ...
    //    imageptrs->ptrimagebitmap = new QImage(11136, 11136, QImage::Format_ARGB32);
    //    QPainter painter(imageptrs->ptrimagebitmap);
    //    QPoint center;
    //    center.setX(opts.geosatellites.at(geoindex).coffhrv);
    //    center.setY(opts.geosatellites.at(geoindex).loffhrv);
    //    painter.setPen(Qt::NoPen);
    //    painter.setBrush(QColor(100, 100, 100));
    //    painter.drawEllipse(center, 5500 - 3, 5500);
    ...
    {
        row_col = (QRgb*)imageptrs->ptrimageGeostationary->scanLine((opts.geosatellites.at(geoindex).rss ? 5 : (this->areatype == 1 ? 24 : 5))*464 - 1 - line);
        //        row_col_bitmap = (QRgb*)imageptrs->ptrimagebitmap->scanLine((opts.geosatellites.at(geoindex).rss ? 5 : (this->areatype == 1 ? 24 : 5))*464 - 1 - line);
        ...
        for (int pixelx = 5568 - 1 ; pixelx >= 0; pixelx--)
        {
            ...
            if(kindofimage == "HRV Color")
            {
                ...
                else
                {
                    ...
                    {
                        int diff = (19*464 + line)/3 * 3712 + UpperEastColumnActual/3 + pixelx/3;
                        int diff1 = (((19*464 + line) * 3712 + UpperEastColumnActual + pixelx)/3);
                        cred = *(pixelsRed + diff);
                        cgreen = *(pixelsGreen + diff);
                        cblue = *(pixelsBlue + diff);
                        if(this->spectrumvector.at(3).length() > 0)
                ...
                if(this->spectrumvector.at(3).length() > 0)
                {
                    valgamma = pow( cnight, gamma) * gammafactor;
                    if (valgamma >= 1024)
                        valgamma = 1023;
                ...
                if(this->spectrumvector.at(3).length() > 0)
                {
                    ...
                    if(ret == -1)
                        row_col[opts.geosatellites[geoindex].imagewidthhrv0 - 1 - pixelx] = qRgb(255, 0, 0);
                    else
                    {
                        observer.SetLocation(latitude, longitude, 0.0);
                        dat.Set(year, month, day, hours, minutes, 0, true);
                        ...
                        else if(elev > - 6.0 && elev < 6.0)
                        {
                            ...
                            {
                                if( (opts.geosatellites[geoindex].rss ? 1392 : 3712) - 100 < opts.geosatellites[geoindex].imageheighthrv0 - 1 - line)
                                    row_col[opts.geosatellites[geoindex].imagewidthhrv0 - 1 - pixelx] = qRgb(0, 0, 0);
                                else
                                    row_col[opts.geosatellites[geoindex].imagewidthhrv0 - 1 - pixelx] = qRgb(red, green, blue);
                            }
                            else
    ...
    //    if(imageptrs->ptrimagebitmap != NULL)
    //    {
    //        delete imageptrs->ptrimagebitmap;
    //        imageptrs->ptrimagebitmap = NULL;
    //    }
...
QImage *SegmentListGeostationary::CalculateBitMap(bool HRV)
{
    ...
    {
        center.setX(opts.geosatellites.at(geoindex).coff);
        center.setY(opts.geosatellites.at(geoindex).loff);
    }
