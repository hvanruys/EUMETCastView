...
double
Fraction_of_Day(int hr,int mi,double se)
{
...
Date_Time(double julian_date, struct tm *cdate)
{
  ...
  jtime = (time_t)((julian_date - 2440587.5)*86400.);
  *cdate = *gmtime( &jtime );
...
//             1.886913*sin(TWOPI*(year - 1975)/33);
  ...
  double t = year - 2000;
  delta_et = 62.92 + 0.32217 * t + 0.005589 * (year - 2000) * t * t;
...
/* Reference:  The 1992 Astronomical Almanac, page B6. */
  ...
  TU   = (jd - 2451545.0)/36525;
  GMST = 24110.54841 + TU * (8640184.812866 + TU * (0.093104 - TU * 6.2E-6));
  GMST = Modulus(GMST + SEC_PER_DAY*OMEGA_E*UT,SEC_PER_DAY);
